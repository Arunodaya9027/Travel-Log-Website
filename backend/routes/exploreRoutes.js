...
router.post("/handle-wishlist/:id", async(req, res) => {
  const id = req.params.id;
  const card = await Card.find({_id: id});
  if(!card)
    res.status(404).statusMessage("Data Not Found, Card doesn't exists");
  ...
  const newWishList = await new User(card);
  console.log("req", newWishList);
